/**
 * Tests for Vibe Scale configuration and suppression system.
 */
...
const FIXTURES_DIR = path.join(__dirname, "fixtures/vibescale-config");

describe("Config Loading", () => {
  describe("loadConfig", () => {
    it("should load config from .vibescale.yml file", () => {
...
describe("Suppression Directives", () => {
  describe("parseSuppressionDirectives", () => {
    it("should parse file-level ALL suppression", () => {
      const source = `/* vibescale-ignore-file ALL */
const x = 1;`;
    ...
    it("should parse file-level specific rule suppression", () => {
      const source = `/* vibescale-ignore-file CONSOLE_DEBUG */
console.log("test");`;
    ...
    it("should parse line suppression", () => {
      const source = `const x = fetch("/api"); // vibescale-ignore-line UNSAFE_IO`;
    ...
    it("should parse next-line suppression", () => {
      const source = `// vibescale-ignore-next-line TEMPORARY_HACK
// TODO: fix this`;
    ...
    it("should parse multiple rules in one directive", () => {
      const source = `// vibescale-ignore-next-line UNBOUNDED_QUERY,LOOPED_IO
const items = await db.findMany();`;
    ...
    it("should ignore invalid rule IDs", () => {
      const source = `// vibescale-ignore-line INVALID_RULE,UNSAFE_IO`;
  ...
  describe("isSuppressed", () => {
    ...
    it("should suppress on same line with line directive", () => {
      const source = `const x = 1; // vibescale-ignore-line UNSAFE_IO`;
      const directives = parseSuppressionDirectives(source);
